-- Migration: aggiunge cover_image_url tra i campi restituiti da get_card_balance
...
BEGIN
  ...
  WITH merchant_balances AS (
    ...
    WHERE c.id = $1
    GROUP BY cm.merchant_id, m.name, c.issuing_merchant_id, m.industry, m.logo_url, m.cover_image_url, m.hours
  ),
  checkpoints AS (
    SELECT
