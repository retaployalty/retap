...
BEGIN
  ...
  current_reward AS (
    SELECT 
      bc.cp_merchant_id,
      CASE 
        -- Se c'è un reward riscattabile al checkpoint corrente, mostralo
        WHEN EXISTS (
          SELECT 1
      ...
      END as reward_name,
      CASE 
        -- È riscattabile se c'è un reward al checkpoint corrente
        WHEN EXISTS (
          SELECT 1
          FROM public.checkpoint_steps cs
          WHERE cs.offer_id = bc.offer_id
